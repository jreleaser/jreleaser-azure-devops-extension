...
export async function getJReleaserRelease(
  ...
  try {
    ...
    const releaseUrl: string =
      response.data['assets'].find((asset: any) => asset.name.includes(releaseName))?.browser_download_url || '';
    const chksumUrl: string =
      response.data['assets'].find((asset: any) => asset.name.includes(chksum))?.browser_download_url || '';
