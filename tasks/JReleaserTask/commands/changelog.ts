...
export class JReleaserChangelog extends AbstractModelCommand {
  constructor(toolrunner: toolrunner.ToolRunner) {
    super(toolrunner);
  }

  protected setup(ctx: ITaskContext): void {
    this.setCommand('changelog');
    if (ctx.dryRun) {
  ...
  exec(): Promise<CommandResponse> {
    ...
    if (runnerResult.code === 0) {
      return Promise.resolve(new CommandResponse(CommandStatus.Success, 'JReleaser changelog successfully'));
    } else {
